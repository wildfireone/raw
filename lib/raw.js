          ...
          s.values.forEach(function (t){
            ti = getNodeIndex(n, t.key, tg)
						...
						for(var i=0; i< t.values.length;i++){
							...
							cidx = 		t.values[i]["chash"];
							pidx = 		t.values[i]["phash"];
							vidx = 		t.values[i]["vhash"];
							lidx = 		t.values[i]["id"];
							multi =	 	t.values[i]["multi"];
							vcount = 	t.values[i]["voyageCount"];
							sortid = 	t.values[i]['sortid'];


							if (ti == -1) {
	              n.push({ name : t.key, group : tg, sortid:sortid })
	              ti = n.length-1;
	            }
	            var value = 1;//size() ? d3.sum(t.values, function (d){ return +size(d); }) : t.values.length;
	            var link = { source : n[si], target : n[ti], value : value, cidx : cidx, pidx : pidx, vidx:vidx, lidx:lidx, multi:multi, vcount:vcount, sortid:sortid};
