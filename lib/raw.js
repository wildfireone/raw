    ...
    graph.map(function (data){
      ...
      for (var i=0; i < steps().length-1; i++ ) {
        ...
        relations.forEach(function (s){
          ...
          if ( si == -1) {
						pidx = s.values[0].values[0]["CompID"];
						tidx = s.values[0].values[0]["platformKey"];
						lidx = s.values[0].values[0]["_id"];

            n.push({ name : s.key, group : sg, pidx : pidx, tidx : tidx, lidx:lidx})
          ...
          s.values.forEach(function (t){
            ti = getNodeIndex(n, t.key, tg)
						pidx = t.values[0]["CompID"];
						tidx = t.values[0]["platformKey"];
						lidx = t.values[0]["_id"];
            if (ti == -1) {
              n.push({ name : t.key, group : tg, pidx : pidx, tidx : tidx, lidx:lidx})
              ti = n.length-1;
            }
            var value = size() ? d3.sum(t.values, function (d){ return +size(d); }) : t.values.length;
            var link = { source : n[si], target : n[ti], value : value, pidx : pidx, tidx : tidx, lidx:lidx};
