    ...
    graph.map(function (data){
      ...
      for (var i=0; i < steps().length-1; i++ ) {
        ...
        relations.forEach(function (s){
          ...
          if ( si == -1) {
            n.push({ name : s.key, group : sg, parent: s.key })
            si = n.length-1;
						if(i==0){
							pidx = s.key;
						}
          ...
          s.values.forEach(function (t){
            ...
            if (ti == -1) {
              n.push({ name : t.key, group : tg, parent:pidx })
              ti = n.length-1;
							if(i+1==steps().length-1){
							 ...
							 n[ti].parent = n[ti].parent + " " + pidx;
								var pidxa = n[ti].parent.split(' ');
								pidx = pidxa[pidxa.length-1]

						}
            var value = size() ? d3.sum(t.values, function (d){ return +size(d); }) : t.values.length;
            var link = { source : n[si], target : n[ti], value : value, pidx : pidx, tidx : tidx};
